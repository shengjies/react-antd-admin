import React,{Component} from 'react'
import {Layout,Menu, Icon, Badge,Avatar,Dropdown,Tabs} from 'antd'
import Loadable from 'react-loadable'
import 'nprogress/nprogress.css'
import 'antd/dist/antd.css';
import './index.css';
import {HashRouter,Route,Link} from 'react-router-dom'
const {Header,Sider,Content} = Layout;
const SubMenu = Menu.SubMenu;
const TabPane = Tabs.TabPane;
const LoadableAdmin = Loadable({
    loader: () => import('../setting/admin'),
    loading: ()=>(''),
  })
  const LoadableRole = Loadable({
    loader: () => import('../setting/role'),
    loading: ()=>(''),
  })
  const LoadableMenu = Loadable({
    loader: () => import('../setting/menu'),
    loading: ()=>(''),
  })
  const LoadableEdit = Loadable({
    loader: () => import('../setting/editpwd'),
    loading: ()=>(''),
  })
  const LoadableMain = Loadable({
    loader: () => import('../main'),
    loading: ()=>(''),
  })
export default class LayoutView extends Component{
    loaderComponent=()=>{
        return Loadable({
          loader: () => import('../setting/admin'),
          loading: ()=>(''),
        })
      }
      componentDidMount(){
        window.addEventListener('resize',this.handleHeight)
      }
      handleHeight=()=>{
        const x = document.documentElement.clientWidth;
        this.setState({
          collapsed: x <=800?true:false
        })
      }
      constructor(props){
        super(props);
        this.state={
          collapsed:false
        }
      }
      toggleCollapsed = () => {
        this.setState({
          collapsed: !this.state.collapsed,
        });
      }
      render(){
        const msgPanel=(
          <Menu style={{top:22}}>
          <Menu.Item>
              <Tabs size="small" defaultActiveKey="1" style={{width:300}}>
                <TabPane tab="消息(11)" key="1">Tab 1</TabPane>
              </Tabs>
            </Menu.Item>
          </Menu>
        )
        const userSetting = (
          <Menu style={{top:22}}>
            <Menu.Item>
              <Icon type="user" />
              <span>个人中心</span>
            </Menu.Item>
            <Menu.Item>
              <Icon type="setting" />
              <span>个人设置</span>
            </Menu.Item>
            <Menu.Divider />
            <Menu.Item>
              <Icon type="logout" />
              <span>退出登录</span>
            </Menu.Item>
          </Menu>
        );
        return(
            <HashRouter props={this.props}>
            <Layout>
              <Sider 
              collapsed={this.state.collapsed}
              style={{background:"#001529",height:'100vh'}}>
              <div className="logo">
                <img className="img" src="data:image/svg+xml;base64," />
                <h1>XXX 系统</h1>
              </div>
              <Menu
                theme="dark"
                defaultOpenKeys={['sub1']}
                defaultSelectedKeys={['/home']}
                mode="inline"
              >
                 <Menu.Item key="/home">
                  <Link to="/home">
                    <Icon type="home" /><span>首页</span>
                  </Link>
                </Menu.Item>
                <SubMenu key="sub1" title={<span><Icon type="setting" /><span>后台设置</span></span>}>
                  <Menu.Item key="2"><Link to="/home/admin">用户信息</Link></Menu.Item>
                  <Menu.Item key="3"><Link to="/home/role">角色信息</Link></Menu.Item>
                  <Menu.Item key="4"><Link to="/home/menu">菜单设置</Link></Menu.Item>
                  <Menu.Item key="5"><Link to="/home/edit">修改密码</Link></Menu.Item>
                </SubMenu>
                <SubMenu key="sub2" title={<span><Icon type="user" /><span>个人页</span></span>}>
                  <Menu.Item key="6"><Link to="/home/admin">个人中心</Link></Menu.Item>
                  <Menu.Item key="7"><Link to="/home/role">个人设置</Link></Menu.Item>
                </SubMenu>
              </Menu>
              </Sider>
              <Layout>
                <Header style={{background:"#fff",paddingLeft:15,paddingRight:15}}>
                    <Icon style={{fontSize:25,cursor:'pointer',lineHeight:3}} onClick={this.toggleCollapsed} type={this.state.collapsed ? 'menu-unfold' : 'menu-fold'} />
                    <div className="antd-pro-components-global-header-index-right">
                      <span className="msg">
                      <Dropdown overlay={msgPanel} placement="bottomLeft" trigger={['click']}>
                        <Badge count={11}>
                          <Icon style={{fontSize:20}} type="message" />
                        </Badge>
                      </Dropdown>
                      </span>
                      <Dropdown overlay={userSetting} placement="bottomLeft">
                        <span className="user">
                          <Avatar src="https://zos.alipayobjects.com/rmsportal/ODTLcjxAfvqbxHnVXCYX.png" />
                          <span> 邋遢 </span>
                        </span>
                      </Dropdown>
                    </div>
                </Header>
                <Content>
                  <Route  path="/home" exact component={LoadableMain}/>
                  <Route  path="/home/admin" exact component={LoadableAdmin}/>
                  <Route  path="/home/role" exact component={LoadableRole}/>
                  <Route  path="/home/menu" exact component={LoadableMenu}/>
                  <Route  path="/home/edit" exact component={LoadableEdit}/>
                </Content>
              </Layout>
            </Layout>
          </HashRouter>
        )
      }
}